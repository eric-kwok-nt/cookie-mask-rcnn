...
- Artifacts for specific runs will be uploaded to a directory with a
  ...
  `gs://mask-rcnn-training-artifacts/mlflow-tracking-server/eric_kwok/<MLFLOW_EXPERIMENT_UUID>`.
...
=== "Linux/macOS"
    ...
    $ cd src
    $ gunicorn mask_rcnn_training_fastapi.main:APP -b 0.0.0.0:8080 -w 4 -k uvicorn.workers.UvicornWorker
    ```
...
=== "Windows PowerShell"
    ...
    $ conda activate mask-rcnn-training
    $ cd src
    $ uvicorn mask_rcnn_training_fastapi.main:APP
    ```
...
`src/mask_rcnn_training_fastapi/config.py`. This class contains
...
`src/mask_rcnn_training_fastapi/config.py`:
```python
...
...
=== "Linux/macOS"
    ...
    $ docker build \
        -t asia.gcr.io/$GCP_PROJECT_ID/eric_kwok/fastapi-server:0.1.0 \
        --build-arg PRED_MODEL_UUID="$PRED_MODEL_UUID" \
        -f docker/mask-rcnn-training-fastapi.Dockerfile \
...
=== "Linux/macOS"
    ...
    $ docker run --rm -p 8080:8080 \
        ...
        asia.gcr.io/$GCP_PROJECT_ID/eric_kwok/fastapi-server:0.1.0
    ```
...
```bash
$ docker push asia.gcr.io/$GCP_PROJECT_ID/eric_kwok/fastapi-server:0.1.0
```
...
=== "Local Machine"
    ...
    ```bash
    $ kubectl port-forward service/fastapi-server-eric-kwok-svc 8080:8080 --namespace=polyaxon-v1
...
!!! attention
    ...
    === "Local Machine"
        ...
        ```bash
        $ kubectl delete fastapi-server-eric-kwok-deployment --namespace=polyaxon-v1
        $ kubectl delete fastapi-server-eric-kwok-svc --namespace=polyaxon-v1
        ```
