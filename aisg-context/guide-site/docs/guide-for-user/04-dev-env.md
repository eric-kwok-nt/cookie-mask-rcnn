...
`docker/mask-rcnn-training-poly-vscode.Dockerfile` to build a
...
=== "Linux/macOS"
    ...
    $ docker build \
        -t asia.gcr.io/$GCP_PROJECT_ID/eric_kwok/vscode-server:0.1.0 \
        -f docker/mask-rcnn-training-poly-vscode.Dockerfile \
        --platform linux/amd64 .
    $ docker push asia.gcr.io/$GCP_PROJECT_ID/eric_kwok/vscode-server:0.1.0
    ```
...
=== "Linux/macOS"
    ...
    $ polyaxon run -f aisg-context/polyaxon/polyaxonfiles/vscode-service.yml \
        -P DOCKER_IMAGE="asia.gcr.io/$GCP_PROJECT_ID/eric_kwok/vscode-server:0.1.0" \
...
`http://localhost:8117/ui/default/mask-rcnn-training-<YOUR_NAME>/services`.
...
=== "Linux/macOS"
    ...
    $ export GCP_PROJECT_ID=cvhub-312105
    $ docker build \
        -t asia.gcr.io/$GCP_PROJECT_ID/eric_kwok/jupyter-server:0.1.0 \
        -f docker/mask-rcnn-training-poly-jupyter.Dockerfile \
        --platform linux/amd64 .
    $ docker push asia.gcr.io/$GCP_PROJECT_ID/eric_kwok/jupyter-server:0.1.0
    ```
...
=== "Linux/macOS"
    ...
    $ polyaxon run -f aisg-context/polyaxon/polyaxonfiles/jupyter-service.yml \
        -P DOCKER_IMAGE="asia.gcr.io/$GCP_PROJECT_ID/eric_kwok/jupyter-server:0.1.0" \
        -P WORKING_DIR="/polyaxon-v1-data" \
        -p mask-rcnn-training-<YOUR_NAME>
    ```
