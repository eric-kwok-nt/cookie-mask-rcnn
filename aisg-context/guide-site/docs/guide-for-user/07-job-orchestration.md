...
(`src/mask_rcnn_training/general_utils.py`)
...
a Dockerfile (`docker/mask-rcnn-training-data-prep.Dockerfile`)
...
=== "Linux/macOS"
    ...
    $ docker build \
        -t asia.gcr.io/$GCP_PROJECT_ID/eric_kwok/data-prep:0.1.0 \
        -f docker/mask-rcnn-training-data-prep.Dockerfile \
        --platform linux/amd64 .
    $ docker push asia.gcr.io/$GCP_PROJECT_ID/eric_kwok/data-prep:0.1.0
    ```
...
=== "Linux/macOS"
    ...
    $ polyaxon run -f aisg-context/polyaxon/polyaxonfiles/process-data.yml \
        -P DOCKER_IMAGE="asia.gcr.io/$GCP_PROJECT_ID/eric_kwok/data-prep:0.1.0" \
        ...
        -P WORKING_DIR="/home/aisg/mask-rcnn-training" \
...
In the module `src/mask_rcnn_training/general_utils.py`,
...
!!! important
    ...
    following name: `gs://mask-rcnn-training-artifacts`.
    ...
    will be
    `gs://mask-rcnn-training-artifacts/mlflow-tracking-server`.

=== "Local Machine / Polyaxon VSCode Terminal"

    ```bash
    $ gsutil mb -p cvhub-312105 -c STANDARD -l ASIA-SOUTHEAST1 -b on gs://mask-rcnn-training-artifacts
    Creating gs://mask-rcnn-training-artifacts/...
    ```
...
`gs://mask-rcnn-training-artifacts/mlflow-tracking-server/eric_kwok`.
For name of experiment, you can specify a name like so:
`ml-experiment-eric_kwok`.
...
=== "Linux/macOS"
    ...
    ```bash
    $ docker build \
        -t asia.gcr.io/$GCP_PROJECT_ID/eric_kwok/model-train:0.1.0 \
        -f docker/mask-rcnn-training-model-training-gpu.Dockerfile \
        --platform linux/amd64 .
    $ docker push asia.gcr.io/$GCP_PROJECT_ID/eric_kwok/model-train:0.1.0
    ```
...
=== "Linux/macOS"
    ...
    $ polyaxon run -f aisg-context/polyaxon/polyaxonfiles/train-model-gpu.yml \
        -P DOCKER_IMAGE="asia.gcr.io/$GCP_PROJECT_ID/eric_kwok/model-train:0.1.0" \
