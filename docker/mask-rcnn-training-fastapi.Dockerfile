...
ARG REPO_DIR="."
ARG CONDA_ENV_FILE="mask-rcnn-training-conda-env.yml"
ARG CONDA_ENV_NAME="mask-rcnn-training"
...
ARG PRED_MODEL_GCS_URI="gs://mask-rcnn-training-artifacts/mlflow-tracking-server/eric_kwok/$PRED_MODEL_UUID"
...
RUN $CONDA_BIN env create -f mask-rcnn-training/$CONDA_ENV_FILE && \
    $CONDA_BIN init bash && \
    $CONDA_BIN clean -a -y && \
...
WORKDIR $HOME_DIR/mask-rcnn-training
